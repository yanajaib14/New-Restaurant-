...
if (DB_CONNECTION) {
  pool = new Pool({ connectionString: DB_CONNECTION });
} else {
  console.warn("WARNING: DATABASE_URL not set. Database features will not work.");
  console.warn("Set DATABASE_URL in your .env file to enable persistence.");
}

const SESSION_SECRET = process.env.SESSION_SECRET || "restaurant-planner-dev-secret-change-in-production";
...
async function startServer() {
  ...
  if (!pool) {
    console.log("Running in frontend-only mode (no database). Data will not persist between sessions.");
  }
  ...
  if (pool) {
    ...
    const createIndexIfNotExists = async (name: string, table: string, column: string) => {
      try {
        await pool!.query(`CREATE INDEX IF NOT EXISTS ${name} ON ${table} (${column})`);
      } catch (e) {
        console.warn(`Could not create index ${name}:`, e);
      }
    };

    await Promise.all([
      createIndexIfNotExists("idx_tasks_status", "tasks", "status"),
      createIndexIfNotExists("idx_tasks_category", "tasks", "category"),
      createIndexIfNotExists("idx_tasks_priority", "tasks", "priority"),
      createIndexIfNotExists("idx_task_todos_status", "task_todos", "status"),
      createIndexIfNotExists("idx_task_todos_category", "task_todos", "category"),
      createIndexIfNotExists("idx_menu_items_section", "menu_items", "section"),
      createIndexIfNotExists("idx_inventory_items_category", "inventory_items", "category"),
      createIndexIfNotExists("idx_inventory_items_dept", "inventory_items", "department"),
      createIndexIfNotExists("idx_permits_status", "permits", "status"),
      createIndexIfNotExists("idx_notes_tag", "notes", "tag"),
      createIndexIfNotExists("idx_invoices_status", "invoices", "status"),
      createIndexIfNotExists("idx_invoices_category", "invoices", "category"),
      createIndexIfNotExists("idx_candidates_stage", "candidates", "stage"),
      createIndexIfNotExists("idx_activity_logs_user", "activity_logs", "\"user\""),
      createIndexIfNotExists("idx_shopping_items_category", "shopping_list_items", "category"),
      createIndexIfNotExists("idx_calendar_events_date", "calendar_events", "date"),
    ]);
  } else {
    console.log("Skipping database schema creation (no database connection).");
  }
  ...
  // Middleware to surface pool to route handlers
  app.use((req, _res, next) => {
    (req as any).pool = pool;
    (req as any).hasDb = !!pool;
    next();
  });
  ...
  // InsForge Setup
  const insforgeUrl = process.env.VITE_INSFORGE_URL?.replace(/\/$/, "") || "";
  const insforgeKey = process.env.VITE_INSFORGE_KEY || "";

  if (!insforgeUrl || !insforgeKey) {
    console.warn("WARNING: InsForge URL or Key not configured. Database features may not work.");
  }

  const insforge = new InsForgeClient({
    baseUrl: insforgeUrl || "https://arqsta8h.us-west.insforge.app",
    anonKey: insforgeKey || "ik_d3902514ce290fc64bb4900a677890ce",
  });
  ...
  // Middleware for routes that require database - returns graceful error if no DB
  const requireDb: express.RequestHandler = (req, res, next) => {
    if (!pool) {
      return res.status(503).json({ ok: false, error: "Database not available. Set DATABASE_URL in your .env file." });
    }
    next();
  };

  app.post("/api/db/write", requireDb, async (req, res) => {
  ...
  app.post("/api/db/read", requireDb, async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: error?.message || "DB read failed" });
    }
  });

  app.get("/api/access/check", requireDb, async (req, res) => {
  ...
  app.post("/api/access/upsert-login", requireDb, async (req, res) => {
  ...
  app.get("/api/access/users", requireDb, async (_req, res) => {
    try {
      const result = await pool.query(
        `
  ...
  app.post("/api/access/revoke", requireDb, async (req, res) => {
    const email = String(req.body?.email || "").trim().toLowerCase();
  ...
  app.post("/api/access/restore", requireDb, async (req, res) => {
    const email = String(req.body?.email || "").trim().toLowerCase();
    if (!email) return res.status(400).json({ error: "Missing email" });
  ...
  app.get("/api/settings/title", requireDb, async (_req, res) => {
    try {
  ...
  app.post("/api/settings/title", requireDb, async (req, res) => {
  ...
  app.post("/api/auth/change-password", async (req, res) => {
    ...
    try {
      ...
      return res.json({
        ok: true,
        message: "Password verified. Note: To change password, use the InsForge dashboard or contact support.",
