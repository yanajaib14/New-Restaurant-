...
export function CandidateModal({ candidate, initialDate, positions, onSave, onClose, userRole }: { candidate: Candidate | null, initialDate?: string, positions: Position[], onSave: (form: Candidate) => void, onClose: () => void, userRole?: string }) {
  ...
  return (
    <Modal title={candidate ? "Edit Candidate" : "New Applicant"} onClose={onClose} width={500}>
      ...
      <Field label="SHARED FEEDBACK (PUBLIC TO TEAM)">
        <textarea 
          value={form.feedback} 
      ...
      {!isManager && (
        <Field label="TEAM NOTES (PRIVATE OWNER DISCUSSION)">
          <textarea 
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
        {candidate && (
          <div style={{ marginRight: "auto" }}>
            <Btn onClick={() => onSave({ ...form, _delete: true } as any)} variant="outline" style={{ color: T.red, borderColor: T.redBorder }} title="Delete candidate"><Trash2 size={13} /></Btn>
          </div>
        )}
        <Btn onClick={onClose} variant="ghost">Cancel</Btn>
                  ...
                  <div key={can.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 10, padding: 12, boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
                    ...
                    {can.partnerNotes && !isManager && (
                      <div style={{ fontSize: 10, color: T.text, background: T.goldLight, padding: 6, borderRadius: 6, marginBottom: 10, border: `1px solid ${T.goldBorder}` }}>
                        <span style={{ fontWeight: 700, fontSize: 8 }}>TEAM:</span> {can.partnerNotes.length > 40 ? can.partnerNotes.substring(0, 40) + "..." : can.partnerNotes}
                      </div>
                    )}

                    <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, borderTop: `1px solid ${T.border}`, paddingTop: 8 }}>
                      <button onClick={() => onEditCan(can)} title="Edit" style={{ background: "none", border: "none", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Edit2 size={12} /></button>
                      <button onClick={() => onDeleteCan(can.id)} title="Delete" style={{ background: "none", border: "none", color: T.red, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 16, flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => onEditMember(member.raw)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 8, padding: "4px 8px", cursor: "pointer", color: T.muted, fontSize: 11, display: "flex", alignItems: "center" }}><Edit2 size={12} /></button>
                    <button onClick={() => onDeleteMember(member.raw)} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 8, padding: "4px 8px", cursor: "pointer", color: T.red, fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                  </div>
                </div>
