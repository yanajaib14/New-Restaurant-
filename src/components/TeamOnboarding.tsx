...
const INIT_HOUSE_RULES = [
  { id: 1, category: "Attendance & Punctuality", icon: "🕐", rules: [
  ...
  { id: 2, category: "Uniform & Appearance", icon: "👔", rules: [
  ...
  { id: 4, category: "Food Safety & Hygiene", icon: "🧼", rules: [
  ...
  { id: 5, category: "Cash & POS Procedures", icon: "💳", rules: [
  ...
  { id: 6, category: "Communication & Teamwork", icon: "🤝", rules: [
...
const INIT_ROLES = [
  { id: "server", label: "Server", icon: "🍽️", color: T.blue, colorLight: T.blueLight, colorBorder: T.blueBorder,
  ...
  { id: "bartender", label: "Bartender", icon: "🍹", color: T.purple, colorLight: T.purpleLight, colorBorder: T.purpleBorder,
  ...
  { id: "host", label: "Host / Hostess", icon: "🚪", color: T.green, colorLight: T.greenLight, colorBorder: T.greenBorder,
  ...
  { id: "busser", label: "Busser / Food Runner", icon: "🏃", color: T.orange, colorLight: T.orangeLight, colorBorder: T.orangeBorder,
  ...
  { id: "line_cook", label: "Line Cook", icon: "👨‍🍳", color: T.red, colorLight: T.redLight, colorBorder: T.redBorder,
  ...
  { id: "dishwasher", label: "Dishwasher", icon: "🫧", color: "#4A9AA8", colorLight: "#EEF8FA", colorBorder: "#B0DBE2",
...
export function TeamOnboarding() {
  ...
  const inpS: React.CSSProperties = { width: "100%", background: "#FAFAFA", border: `1px solid ${T.border}`, borderRadius: 7, padding: "8px 11px", color: T.text, fontSize: 12, fontFamily: "'DM Sans',sans-serif" };
  ...
  return (
    <div className="fu">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, margin: "0 0 3px", fontWeight: 700, color: T.text }}>Team Onboarding</h2>
          <p style={{ fontFamily: "'DM Sans',sans-serif", color: T.muted, fontSize: 12, margin: 0 }}>House rules, role responsibilities, and sidework checklists for every team member</p>
        </div>
      </div>
              ...
              return (
                <button key={r.id} onClick={() => { setActiveRole(r.id); setWhenFilter("All"); setExpandedTask(null); }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ fontSize: 10, fontFamily: "'DM Mono',monospace", color: T.muted }}>{done}/{total}</span>
                  </div>
                </button>
              );
            })}
            ...
            <div>
              ...
              <div style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: isMobile ? 14 : 20, marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, fontWeight: 700, color: T.text }}>{role.label}</div>
                  ...
                  {["All", "Opening", "During Service", "Closing"].map(w => {
                    ...
                    return (
                      <button key={w} onClick={() => setWhenFilter(w)}
                        style={{ cursor: "pointer", borderRadius: 20, padding: "5px 12px", fontSize: 11, fontFamily: "'DM Sans',sans-serif", border: `1px solid ${active ? wc.text : T.border}`, background: active ? wc.bg : "#FFF", color: active ? wc.text : T.muted, fontWeight: active ? 600 : 400, transition: "all .15s" }}>
                        {w}
                      </button>
                    );
                  })}
                      ...
                      <div style={{ display: "flex", alignItems: isMobile ? "flex-start" : "center", gap: 12, padding: isMobile ? "11px 12px" : "13px 18px" }}>
                        ...
                        <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                          ...
                          <button onClick={() => setEditingTask(isEdit ? null : { ...task, roleId: role.id })}
                            style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 8px", color: T.muted, cursor: "pointer", fontSize: 11 }}>✎</button>
                          <button onClick={() => deleteTask(role.id, task.id)}
                            style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "3px 8px", color: T.red, cursor: "pointer", fontSize: 11 }}>✕</button>
                        </div>
                      </div>
                      {/* Expanded detail */}
                      {isExp && !isEdit && (
                ...
                {addingTaskTo === role.id ? (
                  ...
                  <div style={{ padding: "10px 18px", borderTop: `1px solid ${T.border}` }}>
                    <button onClick={() => setAddingTaskTo(role.id)}
                      style={{ background: "none", border: `1px dashed ${T.borderStrong}`, borderRadius: 7, padding: "6px 14px", color: T.muted, cursor: "pointer", fontSize: 12, fontFamily: "'DM Sans',sans-serif" }}>+ Add Sidework Task</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {section === "rules" && (
        <div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 18 }}>
            <button onClick={() => setActiveCat(null)} style={{ cursor: "pointer", borderRadius: 20, padding: "5px 12px", fontSize: 11, fontFamily: "'DM Sans',sans-serif", border: `1px solid ${!activeCat ? T.gold : T.border}`, background: !activeCat ? T.goldLight : "#FFF", color: !activeCat ? T.gold : T.muted, fontWeight: !activeCat ? 600 : 400 }}>All Categories</button>
            {houseRules.map(cat => (
              <button key={cat.id} onClick={() => setActiveCat(activeCat === cat.id ? null : cat.id)}
                style={{ cursor: "pointer", borderRadius: 20, padding: "5px 12px", fontSize: 11, fontFamily: "'DM Sans',sans-serif", border: `1px solid ${activeCat === cat.id ? T.gold : T.border}`, background: activeCat === cat.id ? T.goldLight : "#FFF", color: activeCat === cat.id ? T.gold : T.muted, fontWeight: activeCat === cat.id ? 600 : 400 }}>
              ...
              <div key={cat.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden" }}>
                ...
                <div style={{ padding: "14px 18px", background: T.goldLight, borderBottom: `1px solid ${T.goldBorder}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 14, fontWeight: 700, color: T.text }}>{cat.category}</span>
                    <span style={{ fontSize: 10, fontFamily: "'DM Mono',monospace", color: T.muted }}>{cat.rules.length} rules</span>
                  </div>
                </div>
                {/* Rules list */}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 18px" }}>
                        ...
                        <div style={{ display: "flex", gap: 5 }}>
                          ...
                          <button onClick={() => setEditingRule(isEdit ? null : { ...rule, catId: cat.id })}
                            style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 7px", color: T.muted, cursor: "pointer", fontSize: 11 }}>✎</button>
                          <button onClick={() => deleteRule(cat.id, rule.id)}
                            style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "3px 7px", color: T.red, cursor: "pointer", fontSize: 11 }}>✕</button>
                        </div>
                      </div>
                      {isExp && !isEdit && (
                ...
                {addingRuleTo === cat.id ? (
                  ...
                  <div style={{ padding: "10px 18px" }}>
                    <button onClick={() => setAddingRuleTo(cat.id)} style={{ background: "none", border: `1px dashed ${T.borderStrong}`, borderRadius: 7, padding: "5px 12px", color: T.muted, cursor: "pointer", fontSize: 12, fontFamily: "'DM Sans',sans-serif" }}>+ Add Rule</button>
                  </div>
                )}
              </div>
            ))}
                ...
                <div key={r.id} style={{ background: "#FFF", border: `1px solid ${pct === 100 ? r.colorBorder : T.border}`, borderRadius: 12, overflow: "hidden", transition: "border-color .3s" }}>
                  ...
                  <div style={{ padding: "12px 16px", background: pct === 100 ? r.colorLight : T.bg, borderBottom: `1px solid ${pct === 100 ? r.colorBorder : T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", transition: "background .3s" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {pct === 100 && <span style={{ fontSize: 11, color: r.color }}>✓ All done</span>}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 60, height: 4, background: T.border, borderRadius: 2 }}>
                        <div style={{ height: "100%", width: `${pct}%`, background: r.color, borderRadius: 2, transition: "width .3s" }} />
                      </div>
                      <span style={{ fontSize: 11, fontFamily: "'DM Mono',monospace", color: T.muted }}>{done}/{total}</span>
                    </div>
                  </div>
                    ...
                    return (
                      <div key={period}>
                        <div style={{ padding: "6px 16px", background: wc.bg, borderBottom: `1px solid ${wc.border}` }}>
                          <span style={{ fontSize: 10, fontFamily: "'DM Mono',monospace", color: wc.text, letterSpacing: .8 }}>{period.toUpperCase()}</span>
                        </div>
