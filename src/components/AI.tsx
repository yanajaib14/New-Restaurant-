        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 22, display: "flex", flexDirection: "column", gap: 14 }}>
          {messages.map((msg, i) => (
            <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "76%", padding: "12px 16px", borderRadius: 10, background: msg.role === "user" ? T.goldLight : "#F7F7F7", border: `1px solid ${msg.role === "user" ? T.goldBorder : T.border}`, fontSize: 13, lineHeight: 1.7, fontFamily: "'Cormorant Garamond', serif", color: T.text, whiteSpace: "pre-wrap" }}>
                {msg.role === "assistant" && <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: T.gold, marginBottom: 6, letterSpacing: 1 }}>�S� AI ASSISTANT</div>}
                {msg.content}
              </div>
            </div>
          ))}
          ...
          <div style={{ padding: "0 16px 12px 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
            {messages[messages.length - 1].suggestions.map((q: string) => (
              <button key={q} onClick={() => onSend(q)}
                style={{ background: T.goldLight, border: `1px solid ${T.goldBorder}`, borderRadius: 16, padding: "5px 12px", color: T.gold, cursor: "pointer", fontSize: 11, fontWeight: 500, fontFamily: "'Cormorant Garamond', serif", transition: "all .15s" }}>
                {q}
              </button>
            ))}
          </div>
