            ...
            <div key={item.id} style={{ display: "flex", alignItems: "flex-start", marginBottom: 16, position: "relative" }}>
              ...
              <div style={{ background: item.done ? `${pc}0a` : "#FFF", border: `1px solid ${item.done ? pc + "33" : T.border}`, borderRadius: 10, padding: "13px 18px", flex: 1, display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", transition: "all .15s" }}
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <button onClick={e => { e.stopPropagation(); onEdit(item); }} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 7px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><PenLine size={12} /></button>
                  <button onClick={e => { e.stopPropagation(); onDelete(item); }} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "3px 7px", color: T.red, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                </div>
              </div>
            </div>
          );
