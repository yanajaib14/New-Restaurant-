    ...
    <Modal title={task ? "Edit Task" : "New Task"} onClose={onClose}>
      ...
      <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 18, marginTop: 4 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
          <label style={{ fontSize: 11, fontFamily: "'DM Mono',monospace", color: T.muted, letterSpacing: 1.2, fontWeight: 600 }}>SUBTASK CHECKLIST</label>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "28px 1fr 120px 140px 100px 90px 70px", gap: 0, padding: "13px 18px", alignItems: "center", background: "#FFF", transition: "background .1s" }}
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: total > 0 ? 4 : 0 }}>
            ...
            <span style={{ fontSize: 13, color: T.text, fontFamily: "'Inter', sans-serif", fontWeight: 600 }}>{task.task}</span>
            <span style={{ fontSize: 10, color: cc.dot, background: cc.bg, border: `1px solid ${cc.border}`, borderRadius: 12, padding: "2px 8px", fontFamily: "'Inter', sans-serif", fontWeight: 600 }}>{task.category}</span>
          </div>
          {total > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, paddingLeft: 16 }}>
              ...
              <span style={{ fontSize: 10, color: T.muted, fontFamily: "'DM Mono',monospace" }}>{done}/{total}</span>
            </div>
          )}
        </div>
        ...
        <select value={task.status} onChange={e => onStatusChange(task.id, e.target.value)}
          style={{ background: sc.bg, border: `1px solid ${sc.border}`, color: sc.text, borderRadius: 24, padding: "4px 12px", fontSize: 11, fontFamily: "'Inter', sans-serif", cursor: "pointer", outline: "none", fontWeight: 700 }}>
          {["Not Started", "In Progress", "Complete", "Overdue"].map(s => <option key={s}>{s}</option>)}
        </select>
        <div style={{ fontSize: 11, color: T.muted, textAlign: "center", fontFamily: "'DM Mono',monospace" }}>
          {task.assignedTo || "—"}
        </div>
        <div style={{ textAlign: "center" }}>
          <span style={{ fontSize: 10, fontFamily: "'Inter', sans-serif", fontWeight: 700, padding: "3px 10px", borderRadius: 24, background: pc.bg, color: pc.text, border: `1px solid ${pc.border}` }}>{task.priority.toUpperCase()}</span>
        </div>
        <div style={{ display: "flex", gap: 5, justifyContent: "flex-end" }}>
          <button onClick={() => onEdit(task)} style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 12 }}>✎</button>
          <button onClick={() => onDelete(task)} style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "4px 8px", color: T.red, cursor: "pointer", fontSize: 12 }}>✕</button>
        </div>
      </div>
      {open && total > 0 && (
        <div style={{ background: T.bg, borderTop: `1px solid ${T.border}`, padding: "12px 18px 14px 54px" }}>
          <div style={{ fontSize: 10, fontFamily: "'DM Mono',monospace", color: T.subtle, letterSpacing: .8, marginBottom: 10 }}>SUBTASKS — {done}/{total} COMPLETE</div>
          {task.checklist.map(item => (
            <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 8, cursor: "pointer" }} onClick={() => onToggleCheck(task.id, item.id)}>
              <div style={{ width: 15, height: 15, minWidth: 15, border: `2px solid ${item.done ? T.green : T.border}`, borderRadius: 3, background: item.done ? T.greenLight : "#FFF", display: "flex", alignItems: "center", justifyItems: "center", flexShrink: 0, transition: "all .15s" }}>
                {item.done && <span style={{ color: T.green, fontSize: 10 }}>✓</span>}
              </div>
              <span style={{ fontSize: 12, fontFamily: "'DM Sans',sans-serif", color: item.done ? T.muted : T.text, textDecoration: item.done ? "line-through" : "none", userSelect: "none" }}>{item.text}</span>
            </div>
          ))}
        </div>
      )}
