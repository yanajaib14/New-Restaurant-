...
import { Printer, Download, Upload, PenLine, Trash2 } from 'lucide-react';
    ...
    <Modal title={invoice ? "Edit Invoice" : "Smart Import Invoice"} onClose={onClose} width={500}>
      ...
      <Field label="PAID BY">
        <input
          ...
          placeholder="e.g. Owner, Team, Manager"
          style={inpStyle}
        />
      </Field>
          ...
          {fullyFilteredInvoices.length === 0 ? (
            ...
            <div key={inv.id} className="mobile-card" style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 16, padding: "16px 16px 12px", boxShadow: "0 2px 8px rgba(0,0,0,0.03)" }}>
              ...
              <div style={{ display: "flex", gap: 8, borderTop: `1px solid ${T.border}`, paddingTop: 10 }}>
                <button onClick={() => onEdit(inv)} title="Edit" style={{ flex: 1, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 10, padding: "10px", color: T.text, cursor: "pointer", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center" }}><PenLine size={14} /></button>
                <button onClick={() => onDelete(inv)} title="Delete" style={{ flex: 1, background: T.redLight, border: `1px solid ${T.redBorder}`, borderRadius: 10, padding: "10px", color: T.red, cursor: "pointer", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center" }}><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
                ...
                fullyFilteredInvoices.map((inv, i) => (
                  <div key={inv.id} style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 0.9fr 1fr 0.9fr 0.9fr 0.8fr 88px", padding: "13px 20px", borderBottom: `1px solid ${T.border}`, background: i % 2 === 0 ? "#FFF" : T.bg, alignItems: "center" }}
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button onClick={() => onEdit(inv)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 10px", color: T.muted, cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center" }}><PenLine size={13} /></button>
                      <button onClick={() => onDelete(inv)} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "4px 10px", color: T.red, cursor: "pointer", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center" }}><Trash2 size={13} /></button>
                    </div>
                  </div>
                ))
              )}
