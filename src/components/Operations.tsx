...
export function VendorManager({ vendors, onAdd, onEdit, onDelete }: { vendors: Vendor[], onAdd: () => void, onEdit: (v: Vendor) => void, onDelete: (id: number) => void }) {
  const [expandedVendorIds, setExpandedVendorIds] = useState<Record<number, boolean>>({});
  const isMobile = window.innerWidth < 900;

  return (
    <div className="fu">
      <SectionHeader title="Vendor Directory" subtitle="Manage your supply chain and delivery schedules"
        action={<Btn onClick={onAdd} variant="primary" title="Add vendor" style={{ width: 30, minWidth: 30, height: 30, padding: 0, color: "#FFF" }}><span style={{ color: "#FFF", fontSize: 20, fontWeight: 800, lineHeight: 1 }}>+</span></Btn>} />
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
        {vendors.map(v => (
          <div key={v.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: 12 }}>
            <button
              onClick={() => setExpandedVendorIds(prev => ({ ...prev, [v.id]: !prev[v.id] }))}
              style={{ width: "100%", textAlign: "left", background: "none", border: "none", padding: 0, margin: 0, cursor: "pointer" }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: "'Segoe UI', 'Helvetica Neue', Arial, sans-serif", fontSize: 14, fontWeight: 700, color: T.text, overflowWrap: "anywhere" }}>{v.name}</div>
                  <div style={{ marginTop: 3, display: "inline-block", fontSize: 10, color: T.muted, border: `1px solid ${T.border}`, borderRadius: 99, padding: "2px 8px", background: T.bg }}>{v.category}</div>
                </div>
                <div style={{ fontSize: 14, color: T.muted, flexShrink: 0 }}>{expandedVendorIds[v.id] ? "−" : "+"}</div>
              </div>
            </button>
            ...
            {expandedVendorIds[v.id] && (
              <div style={{ borderTop: `1px solid ${T.border}`, marginTop: 10, paddingTop: 10 }}>
                ...
                <div style={{ fontSize: 12, color: T.text, marginBottom: 7 }}>
                  <strong>Email:</strong>{" "}
                  {v.email ? (
                    <a href={`https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(v.email || "")}`} target="_blank" rel="noreferrer" style={{ color: T.text, textDecoration: "underline" }}>{v.email}</a>
                  ) : "-"}
                </div>
                <div style={{ fontSize: 12, color: T.text, marginBottom: 7 }}>
                  <strong>Phone:</strong>{" "}
                  {v.phone ? <a href={`tel:${String(v.phone || "").replace(/[^\d+]/g, "")}`} style={{ color: T.text, textDecoration: "underline" }}>{v.phone}</a> : "-"}
                </div>
                <div style={{ fontSize: 11, color: T.muted, marginBottom: 7 }}>
                  <strong>Delivery:</strong> {v.deliveryDays?.length ? v.deliveryDays.join(", ") : "Not set"}
                </div>
                {v.notes && <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "8px 10px", fontSize: 11, color: T.muted, lineHeight: 1.45, marginBottom: 8 }}>{v.notes}</div>}
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => onEdit(v)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><PenLine size={12} /></button>
                  <button onClick={() => onDelete(v.id)} title="Delete" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                </div>
              </div>
            )}
          </div>
...
// Contacts (non-vendor)
export function ContactManager({ contacts, onAdd, onEdit, onDelete }: { contacts: Contact[], onAdd: () => void, onEdit: (c: Contact) => void, onDelete: (id: number) => void }) {
  const [expandedContactIds, setExpandedContactIds] = useState<Record<number, boolean>>({});
  const isMobile = window.innerWidth < 900;

  return (
    <div className="fu">
      <SectionHeader title="Operations Contacts" subtitle="Track landlord, contractors, legal, permits, and other non-vendor contacts"
        action={<Btn onClick={onAdd} variant="primary" title="Add contact" style={{ width: 30, minWidth: 30, height: 30, padding: 0, color: "#FFF" }}><span style={{ color: "#FFF", fontSize: 20, fontWeight: 800, lineHeight: 1 }}>+</span></Btn>} />
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
        {contacts.map(c => (
          <div key={c.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: 12 }}>
            <button
              onClick={() => setExpandedContactIds(prev => ({ ...prev, [c.id]: !prev[c.id] }))}
              style={{ width: "100%", textAlign: "left", background: "none", border: "none", padding: 0, margin: 0, cursor: "pointer" }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: "'Segoe UI', 'Helvetica Neue', Arial, sans-serif", fontSize: 14, fontWeight: 700, color: T.text, overflowWrap: "anywhere" }}>{c.name}</div>
                  <div style={{ marginTop: 3, display: "inline-block", fontSize: 10, color: T.muted, border: `1px solid ${T.border}`, borderRadius: 99, padding: "2px 8px", background: T.bg }}>{c.category || "General"}</div>
                </div>
                <div style={{ fontSize: 14, color: T.muted, flexShrink: 0 }}>{expandedContactIds[c.id] ? "−" : "+"}</div>
              </div>
            </button>
            ...
            {expandedContactIds[c.id] && (
              <div style={{ borderTop: `1px solid ${T.border}`, marginTop: 10, paddingTop: 10 }}>
                ...
                <div style={{ fontSize: 12, color: T.text, marginBottom: 7 }}>
                  <strong>Email:</strong>{" "}
                  {c.email ? (
                    <a href={`https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(c.email || "")}`} target="_blank" rel="noreferrer" style={{ color: T.text, textDecoration: "underline" }}>{c.email}</a>
                  ) : "-"}
                </div>
                <div style={{ fontSize: 12, color: T.text, marginBottom: 7 }}>
                  <strong>Phone:</strong>{" "}
                  {c.phone ? <a href={`tel:${String(c.phone || "").replace(/[^\d+]/g, "")}`} style={{ color: T.text, textDecoration: "underline" }}>{c.phone}</a> : "-"}
                </div>
                {c.notes && <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "8px 10px", fontSize: 11, color: T.muted, lineHeight: 1.45, marginBottom: 8 }}>{c.notes}</div>}
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <button onClick={() => onDelete(c.id)} title="Delete" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                </div>
              </div>
            )}
          </div>
...
export function ContactModal({ contact, onSave, onClose }: { contact: Contact | null, onSave: (form: any) => void, onClose: () => void }) {
  const blank = { name: "", role: "", company: "", category: "General", email: "", phone: "", notes: "" };
  const [form, setForm] = useState<any>(contact ? { ...contact } : blank);
  const set = (k: string, v: any) => setForm((f: any) => ({ ...f, [k]: v }));

  return (
    <Modal title={contact ? "Edit Contact" : "New Contact"} onClose={onClose} width={500}>
      <Field label="CONTACT NAME"><input value={form.name} onChange={e => set("name", e.target.value)} placeholder="e.g. Alex Rivera" style={inpStyle} /></Field>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field label="ROLE"><input value={form.role} onChange={e => set("role", e.target.value)} placeholder="e.g. Landlord" style={inpStyle} /></Field>
        <Field label="COMPANY"><input value={form.company} onChange={e => set("company", e.target.value)} placeholder="e.g. Broadway Properties" style={inpStyle} /></Field>
      </div>
      <Field label="CATEGORY">
        <select value={form.category} onChange={e => set("category", e.target.value)} style={{ ...inpStyle, height: 42 }}>
          {["General", "Landlord", "Legal", "Contractor", "Consultant", "Permits", "Utilities", "Emergency"].map(cat => (
      ...
      <Field label="NOTES">
        <textarea value={form.notes || ""} onChange={e => set("notes", e.target.value)} placeholder="Best way to reach, hours, responsibilities..." rows={3} style={{ ...inpStyle, resize: "vertical", lineHeight: 1.5 }} />
      </Field>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
        <Btn onClick={onClose} variant="ghost">Cancel</Btn>
        <Btn onClick={() => { if (form.name.trim()) onSave(form); }} variant="primary">{contact ? "Save Changes" : "Add Contact"}</Btn>
      </div>
    </Modal>
  );
}
          ...
          return (
            <div key={item.id} style={{ display: "grid", gridTemplateColumns: "1fr 120px 180px 100px 120px 80px", padding: "14px 18px", borderBottom: `1px solid ${T.border}`, alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                <button onClick={() => onEdit(item)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><PenLine size={12} /></button>
                <button onClick={() => onDelete(item.id)} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "4px 8px", color: T.red, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
              </div>
            </div>
          );
        })}
        ...
        {utilities.map(u => (
          <div key={u.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => onEdit(u)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><PenLine size={12} /></button>
                <button onClick={() => onDelete(u.id)} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "4px 8px", color: T.red, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
              </div>
            </div>
