...
export function PermitModal({ permit, onSave, onClose }: { permit: Permit | null, onSave: (form: any) => void, onClose: () => void }) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        set("fileUrl", event.target?.result);
  ...
  const getFileName = () => {
    ...
    if (form.fileUrl.startsWith("data:")) return "Document uploaded";
    ...
    <Modal title={permit ? "Edit Permit" : "New Permit"} onClose={onClose} width={420}>
      ...
      <Field label="DOCUMENT UPLOAD (Max 5MB)">
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <input type="file" onChange={handleFileUpload} accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.xlsx" disabled={isUploading}
            style={{ flex: 1, ...inpStyle, padding: "8px 12px", opacity: isUploading ? 0.6 : 1, cursor: isUploading ? "not-allowed" : "pointer" }} />
          {form.fileUrl && (
            <button onClick={() => set("fileUrl", "")} disabled={isUploading}
        ...
        {isUploading && <div style={{ fontSize: 11, color: T.blue, marginTop: 8, fontWeight: 600 }}>Processing file...</div>}
        {!isUploading && getFileName() && <div style={{ fontSize: 11, color: T.green, marginTop: 8, fontWeight: 600 }}>Uploaded: {getFileName()}</div>}
      </Field>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
        <Btn onClick={onClose} variant="ghost">Cancel</Btn>
