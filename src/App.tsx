...
export default function App() {
  ...
  const saveNote = async (form: any) => {
    ...
    const { id, ...rest } = form;
    const editId = noteModal && noteModal !== 'new' ? noteModal.id : id;
    try {
      if (editId) {
  ...
  return (
    <div style={{ minHeight:"100dvh", background:T.bg, color:T.text, fontFamily:"'Segoe UI', 'Helvetica Neue', Arial, sans-serif", paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
      ...
      {noteDetail && <NoteDetailModal note={noteDetail} tasks={tasks} onClose={() => setNoteDetail(null)} onSave={async (form) => { await saveNote(form); setNoteDetail(null); }} onDelete={(id) => {
