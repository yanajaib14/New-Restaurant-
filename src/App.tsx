...
import {
  ...
  MenuItem, StartupCost, OperatingCost, Milestone, Note, Vendor, Contact,
  InventoryItem, Permit, MarketingPost, TrainingModule, DailyChecklist,
  UtilityAccount, DigitalAsset, User, ActivityLog, DecisionLog, UserRole, Invoice, Position, Candidate,
  CalendarEvent
} from "./types";
...
import { VendorManager, VendorModal, ContactManager, ContactModal, InventoryTracker, InventoryModal, PermitTracker, PermitModal, UtilityTracker, UtilityModal } from "./components/Operations";
...
const TASKS_STORAGE_KEY = "restaurant_tasks_v1";
const NOTES_STORAGE_KEY = "restaurant_notes_v1";
const CHECKLISTS_STORAGE_KEY = "restaurant_daily_checklists_v1";

const readStoredRows = <T,>(key: string): T[] | null => {
...
const writeStoredRows = (key: string, rows: unknown[]) => {
...
const readStoredTaskTodos = (): TaskTodoItem[] | null => {
  return readStoredRows<TaskTodoItem>(TASK_TODOS_STORAGE_KEY);
};
...
const INIT_CONTACTS: Contact[] = [
  { id: 1, name: "Jordan Lee", role: "Landlord", company: "Broadway Properties", category: "Landlord", email: "jordan@broadwayproperties.com", phone: "555-0191", notes: "Primary building contact for lease and TI access." },
  { id: 2, name: "Mina Patel", role: "Attorney", company: "Patel Legal Group", category: "Legal", email: "mina@patellegal.com", phone: "555-0192", notes: "Review contracts, permits, and operating agreements." },
];
...
const INIT_TASK_TODOS: TaskTodoItem[] = [
  ...
  { id: 3, title: "Draft opening week training roster", category: "Staffing", status: "On Hold", assignedTo: "Team", linkedTaskId: 6, note: "Waiting on final shift budget", created_at: new Date().toISOString() },
];
...
const INIT_CANDIDATES: Candidate[] = [
  { id: 1, name: "Alice Smith", position: "General Manager", stage: "Interviewed", date: "2026-04-16", feedback: "Strong background in upscale dining. Team needs to meet her." },
...
const INIT_ACTIVITY: ActivityLog[] = [
  ...
  { id: 2, user: "Team", action: "Added new candidate: Alice Smith", timestamp: "45 mins ago" },
  ...
  { id: 5, user: "Team", action: "Added 'Truffle Flatbread' to Seasonal Menu", timestamp: "5 hours ago" },
];

const INIT_DECISION_LOGS: DecisionLog[] = [
  { id: 1, title: "POS Vendor Selection", decision: "Proceed with Toast POS for launch.", owner: "Owner", date: "2026-05-20", impact: "High", status: "Decided", nextStep: "Finalize contract and onboarding timeline." },
  { id: 2, title: "Soft Opening Format", decision: "Invite-only soft opening for 3 nights.", owner: "Team", date: "2026-05-22", impact: "Medium", status: "Decided", nextStep: "Publish invite list and staffing plan." },
];

const DECISION_DRAFT_BLANK = {
  title: "",
  decision: "",
  owner: "",
  date: new Date().toISOString().slice(0, 10),
  impact: "Medium" as DecisionLog["impact"],
  status: "Proposed" as DecisionLog["status"],
  nextStep: "",
};

const INIT_ASSETS: DigitalAsset[] = [
  { id: 1, name: "Instagram Brand Account", category: "Social Media", folder: "Brand", assetType: "Link", url: "instagram.com/ourrestaurant", loginInfo: "User: ourrest_admin\nPass: *********", notes: "Main marketing channel", tags: "instagram,brand" },
  { id: 2, name: "Shopify", category: "Software", folder: "Launch Tools", assetType: "Link", url: "shopify.com", loginInfo: "Email: owner@rest.com", notes: "Merch and gift cards", tags: "ecommerce,store" },
  { id: 3, name: "Toast POS", category: "POS", folder: "Launch Tools", assetType: "Link", url: "toasttab.com", loginInfo: "Admin: manager1", notes: "Primary point of sale", tags: "operations,pos" },
  { id: 4, name: "Dropbox Photo Folder", category: "Inspiration", folder: "Photo Sources", assetType: "Link", url: "https://www.dropbox.com/scl/fo/2bqe785ejhqtovanktrb7/AMn-iAg9rAWSmq7yXP7uQyE?rlkey=fm5kpcuktza3p1y2l35kz7huq&st=gjgw1few&dl=0", notes: "Shared Dropbox folder for inspiration photos", tags: "dropbox,photos,inspiration" },
];
...
export default function App() {
  ...
  const [tasks, setTasks]       = useState<Task[]>(() => {
    const stored = readStoredRows<Task>(TASKS_STORAGE_KEY);
    if (stored) return stored;
    return USE_SEED_DATA ? INIT_TASKS : [];
  });
  const [taskTodos, setTaskTodos] = useState<TaskTodoItem[]>(() => {
    const stored = readStoredTaskTodos();
    if (stored && stored.length) return stored.map(sanitizeTaskTodo);
    return USE_SEED_DATA ? INIT_TASK_TODOS.map(sanitizeTaskTodo) : [];
  });
  const [menuItems, setMenuItems] = useState<MenuItem[]>(USE_SEED_DATA ? INIT_MENU : []);
  const [startup, setStartup]   = useState<StartupCost[]>(USE_SEED_DATA ? INIT_STARTUP : []);
  const [operating, setOp]      = useState<OperatingCost[]>(USE_SEED_DATA ? INIT_OPERATING : []);
  const [timeline, setTL]       = useState<Milestone[]>(USE_SEED_DATA ? INIT_TIMELINE : []);
  const [notes, setNotes]       = useState<Note[]>(() => {
    const stored = readStoredRows<Note>(NOTES_STORAGE_KEY);
    if (stored) return stored;
    return USE_SEED_DATA ? INIT_NOTES : [];
  });
  const [vendors, setVendors]   = useState<Vendor[]>(USE_SEED_DATA ? INIT_VENDORS : []);
  const [contacts, setContacts] = useState<Contact[]>(USE_SEED_DATA ? INIT_CONTACTS : []);
  const [inventory, setInventory] = useState<InventoryItem[]>(USE_SEED_DATA ? INIT_INVENTORY : []);
  const [permits, setPermits]   = useState<Permit[]>(USE_SEED_DATA ? INIT_PERMITS : []);
  const [utilities, setUtilities] = useState<UtilityAccount[]>(USE_SEED_DATA ? INIT_UTILITIES : []);
  const [utilityModal, setUtilityModal] = useState<UtilityAccount | "new" | null>(null);
  const [marketing, setMarketing] = useState<MarketingPost[]>(USE_SEED_DATA ? INIT_MARKETING : []);
  const [training, setTraining]   = useState<TrainingModule[]>(USE_SEED_DATA ? INIT_TRAINING : []);
  const [checklists, setChecklists] = useState<DailyChecklist[]>(() => {
    const stored = readStoredRows<DailyChecklist>(CHECKLISTS_STORAGE_KEY);
    if (stored) return stored;
    return USE_SEED_DATA ? INIT_CHECKLISTS : [];
  });
  const [calendarEvents, setCalendarEvents] = useState<CalendarEvent[]>([]);
  const [decisionLogs, setDecisionLogs] = useState<DecisionLog[]>(USE_SEED_DATA ? INIT_DECISION_LOGS : []);
  ...
  const [decisionModal, setDecisionModal] = useState<DecisionLog | "new" | null>(null);
  const [decisionDraft, setDecisionDraft] = useState<any>(DECISION_DRAFT_BLANK);
  ...
  const [positions, setPositions]   = useState<Position[]>(USE_SEED_DATA ? INIT_POSITIONS : []);
  const [candidates, setCandidates] = useState<Candidate[]>(USE_SEED_DATA ? INIT_CANDIDATES : []);
  const [assets, setAssets]         = useState<DigitalAsset[]>(USE_SEED_DATA ? INIT_ASSETS : []);
  ...
  const tableBackupKeys: Record<string, string> = {
    tasks: TASKS_STORAGE_KEY,
    notes: NOTES_STORAGE_KEY,
    daily_checklists: CHECKLISTS_STORAGE_KEY,
  };
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      const tableBindings: Array<{ table: string; set: (rows: any[]) => void }> = [
        ...
        { table: "contacts", set: setContacts as any },
        ...
        { table: "decision_logs", set: setDecisionLogs as any },
      ];
      ...
      await Promise.all(
        tableBindings.map(async ({ table, set }) => {
          ...
          if (!error && Array.isArray(data)) {
            const backupKey = tableBackupKeys[table];
            if (!backupKey) {
              set(data as any);
              return;
            }
            ...
            if (data.length > 0) {
              writeStoredRows(backupKey, data as unknown[]);
              set(data as any);
              return;
            }

            const stored = readStoredRows<any>(backupKey);
      ...
      if (!todoError && Array.isArray(todoRows)) {
        const normalized = ((todoRows as TaskTodoItem[]) || []).map(sanitizeTaskTodo);
        if (normalized.length > 0) {
          setTaskTodos(normalized);
          writeStoredRows(TASK_TODOS_STORAGE_KEY, normalized as unknown[]);
          setTaskTodoStore("db");
        } else {
          const stored = readStoredTaskTodos();
          if (stored && stored.length > 0) {
            setTaskTodos(stored.map(sanitizeTaskTodo));
  ...
  useEffect(() => {
    ...
    writeStoredRows(TASK_TODOS_STORAGE_KEY, taskTodos as unknown[]);
  }, [taskTodos, hasLoadedInitialData]);

  useEffect(() => {
    if (!hasLoadedInitialData) return;
    writeStoredRows(TASKS_STORAGE_KEY, tasks as unknown[]);
  }, [tasks, hasLoadedInitialData]);

  useEffect(() => {
    if (!hasLoadedInitialData) return;
    writeStoredRows(NOTES_STORAGE_KEY, notes as unknown[]);
  }, [notes, hasLoadedInitialData]);

  useEffect(() => {
    if (!hasLoadedInitialData) return;
    writeStoredRows(CHECKLISTS_STORAGE_KEY, checklists as unknown[]);
  }, [checklists, hasLoadedInitialData]);
  ...
  const [dependencyLockEnabled, setDependencyLockEnabled] = useState(true);
  ...
  const taskPriorityRank = (priority?: string) => {
    const order: Record<string, number> = { Critical: 0, High: 1, Medium: 2, Low: 3 };
    return order[priority || ""] ?? 99;
  };
  const sortTasksForSequence = (a: Task, b: Task) => {
    const dueDelta = taskDueRank(a.due) - taskDueRank(b.due);
  ...
  const categoryTimelineCards = CATEGORIES.map(category => {
    ...
    const done = categoryTasks.filter(task => task.status === "Complete").length;
    const open = total - done;
    const nextDueTask = categoryTasks.find(task => task.status !== "Complete" && task.due);
    return {
      category,
      total,
      done,
      ...
      nextDue: nextDueTask?.due || "",
    };
  }).filter(card => card.total > 0);
  const blockedTaskReasonById = !dependencyLockEnabled ? {} as Record<number, string> : CATEGORIES.reduce((acc, category) => {
    const categoryTasks = [...tasks.filter(task => task.category === category)].sort(sortTasksForSequence);
    const firstIncompleteIndex = categoryTasks.findIndex(task => task.status !== "Complete");
    if (firstIncompleteIndex < 0) return acc;
    const blocker = categoryTasks[firstIncompleteIndex];
    categoryTasks.forEach((task, idx) => {
      if (idx > firstIncompleteIndex && task.status !== "Complete") {
        acc[task.id] = `Waiting for \"${blocker.task}\" to complete first.`;
      }
    });
    return acc;
  }, {} as Record<number, string>);
  const isTaskSequenceBlocked = (task: Task) => Boolean(blockedTaskReasonById[task.id]);
  ...
  const weeklyWins = useMemo(() => {
    const wins: string[] = [];
    if (completed > 0) wins.push(`${completed} master task(s) completed`);
    if (doneTaskTodos.length > 0) wins.push(`${doneTaskTodos.length} action-list todo(s) completed`);
    if (staffingProg > 0) wins.push(`Staffing progress at ${staffingProg}% (${totHired}/${totOpenings} hires)`);
    const activePermits = permits.filter(p => p.status === "Active").length;
    if (activePermits > 0) wins.push(`${activePermits} permit(s) currently active`);
    return wins.slice(0, 4);
  }, [completed, doneTaskTodos.length, staffingProg, totHired, totOpenings, permits]);
  ...
  const weeklyBlockers = useMemo(() => {
    ...
    tasks
      ...
      .slice(0, 3)
      .forEach(t => blockers.push(`Overdue task: ${t.task} (${t.category})`));
    permitAlerts
      .filter(x => (x.daysLeft as number) <= 14)
      .slice(0, 2)
      .forEach(x => blockers.push(`Permit risk: ${x.permit.name} (${x.daysLeft as number} day(s) left)`));
    openTaskTodos
      .filter(todo => todo.status === "Not Started")
      .slice(0, 2)
      .forEach(todo => blockers.push(`Not started todo: ${todo.title}`));
    return blockers.slice(0, 5);
  }, [tasks, permitAlerts, openTaskTodos]);

  const weeklyNextUp = useMemo(() => {
    const dueSoon = tasks
      .filter(t => t.status !== "Complete")
      .map(t => ({ ...t, dueDate: parsePlannerDate(t.due) }))
      .filter(t => t.dueDate && ((t.dueDate as Date).getTime() - startOfToday.getTime()) / (1000 * 60 * 60 * 24) <= 7)
      .sort((a, b) => ((a.dueDate as Date).getTime() - (b.dueDate as Date).getTime()))
      .slice(0, 4)
      .map(t => `${t.task} (${t.due || "Due soon"})`);

    const todoFocus = actionListTodos
      .filter(todo => todo.status !== "Done")
      .slice(0, 2)
      .map(todo => `${todo.title} (${todo.category})`);
  ...
  const weeklyPartnerBriefText = useMemo(() => {
    ...
    lines.push("");
    lines.push(`Progress: ${prog}% complete`);
    lines.push(`Overdue tasks: ${overdue}`);
    lines.push(`Critical overdue: ${criticalOverdue}`);
    lines.push(`Staffing: ${totHired}/${totOpenings} hired (${staffingProg}%)`);
    lines.push("");
    lines.push("Wins:");
    (weeklyWins.length ? weeklyWins : ["No major wins logged this period."]).forEach(item => lines.push(`- ${item}`));
    lines.push("");
    lines.push("Blockers:");
    (weeklyBlockers.length ? weeklyBlockers : ["No active blockers right now."]).forEach(item => lines.push(`- ${item}`));
    lines.push("");
    lines.push("Next 7 Days:");
    (weeklyNextUp.length ? weeklyNextUp : ["No due-soon actions identified."]).forEach(item => lines.push(`- ${item}`));
    return lines.join("\n");
  }, [prog, overdue, criticalOverdue, totHired, totOpenings, staffingProg, weeklyWins, weeklyBlockers, weeklyNextUp]);

  const partnerUpdates = useMemo(() => {
    return notes
      .filter(n => {
        if (n.tag !== "Operations") return false;
        const title = String(n.title || "");
        return /(^|\s)Update\s-/i.test(title);
      })
  ...
  const extractWriteRow = (result: any) => {
    if (Array.isArray(result?.data) && result.data[0]) return result.data[0];
    if (result?.data?.row) return result.data.row;
  ...
  const pushCategoryTimelineBackFromTask = async (sourceTask: Task, days: number) => {
    if (!Number.isFinite(days) || days <= 0) return;
    const chain = [...tasks.filter(task => task.category === sourceTask.category)].sort(sortTasksForSequence);
    const startIdx = chain.findIndex(task => task.id === sourceTask.id);
    if (startIdx < 0) return;

    const affected = chain.slice(startIdx).filter(task => !!task.due);
    if (affected.length === 0) {
      alert("No dated tasks found from this point in the sequence.");
      return;
    }

    try {
      await Promise.all(
        affected.map(task => dbUpdate("tasks", task.id, { due: shiftIsoDateByDays(task.due, days) }))
      );
      await refetch("tasks", setTasks);
      logActivity(`Shifted ${affected.length} ${sourceTask.category} tasks by ${days} day(s) from ${sourceTask.task}`);
    } catch (e: any) {
      console.error("Timeline Delay Error:", e);
      alert(`Failed to shift task timeline: ${e.message || JSON.stringify(e)}`);
    }
  };

  const promptDelayFromTask = async (task: Task) => {
    const input = window.prompt(`How many days should this category timeline move back from \"${task.task}\" onward?`, "1");
    ...
    await pushCategoryTimelineBackFromTask(task, Math.floor(days));
  };
  ...
  const saveTask = async (form: any) => {
    ...
    try {
      if (_delete) {
        ...
        const result = await dbUpdate('tasks', taskModal.id, rest);
        const { error } = result;
        if (error) throw error;
        const optimistic = extractWriteRow(result) || { id: taskModal.id, ...rest };
        setTasks(prev => prev.map(task => task.id === taskModal.id ? (optimistic as Task) : task));
        logActivity(`Updated task: ${form.task}`);
      } else {
        const result = await dbInsert('tasks', rest);
        const { error } = result;
        if (error) throw error;
        const optimistic = extractWriteRow(result) || { id: Date.now(), ...rest };
  ...
  const saveNote = async (form: any): Promise<boolean> => {
    ...
    const { id, ...rest } = form;
    try {
      const editingId = (noteModal && noteModal !== 'new') ? noteModal.id : (id || null);
      if (editingId) {
        const result = await dbUpdate('notes', editingId, rest);
        const { error } = result;
        if (error) throw error;
        const optimistic = extractWriteRow(result) || { id: editingId, ...rest };
        setNotes(prev => prev.map(note => note.id === editingId ? (optimistic as Note) : note));
      } else {
        const result = await dbInsert('notes', { ...rest, date: d });
        const { error } = result;
        if (error) throw error;
        const optimistic = extractWriteRow(result) || { id: Date.now(), ...rest, date: d };
  ...
  const saveDecision = async (form: any) => {
    const { id, ...rest } = form;
    try {
      if (decisionModal && decisionModal !== "new") {
        const { error } = await dbUpdate("decision_logs", decisionModal.id, rest);
        if (error) throw error;
        logActivity(`Updated decision: ${form.title}`);
      } else {
        const { error } = await dbInsert("decision_logs", rest);
        if (error) throw error;
        logActivity(`Added decision: ${form.title}`);
      }
      await refetch("decision_logs", setDecisionLogs as any);
      setDecisionModal(null);
    } catch (e: any) {
      console.error("Save Decision Error:", e);
      alert(`Failed to save decision: ${e.message || JSON.stringify(e)}`);
    }
  };

  const openNewDecision = () => {
    setDecisionDraft({ ...DECISION_DRAFT_BLANK, date: new Date().toISOString().slice(0, 10) });
    setDecisionModal("new");
  };

  const openEditDecision = (item: DecisionLog) => {
    setDecisionDraft({ ...item });
  ...
  const saveContact = async (form: any) => {
    ...
    try {
      if (contactModal && contactModal !== 'new') {
        const { error } = await dbUpdate('contacts', contactModal.id, rest);
        if (error) throw error;
      } else {
        const { error } = await dbInsert('contacts', rest);
        if (error) throw error;
      }
      await refetch('contacts', setContacts);
      setContactModal(null);
    } catch (e: any) {
      console.error("Save Contact Error:", e);
      alert(`Failed to save contact: ${e.message || JSON.stringify(e)}`);
    }
  };
  ...
  const copyWeeklyPartnerBrief = async () => {
    try {
      await navigator.clipboard.writeText(weeklyPartnerBriefText);
      alert("Weekly team brief copied to clipboard.");
    } catch (e) {
      console.error("Copy team brief failed", e);
      alert("Could not copy brief. Please try again.");
    }
  };

  const saveWeeklyPartnerBriefToNotes = async () => {
    try {
      const dateLabel = new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "America/Phoenix" });
      const payload = {
        tag: "Operations",
        title: `Weekly Team Brief - ${dateLabel}`,
        body: weeklyPartnerBriefText,
        date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'America/Phoenix' }),
        files: [],
      };
      const { error } = await dbInsert("notes", payload as any);
      if (error) throw error;
      await refetch("notes", setNotes);
      logActivity(`Saved weekly team brief (${dateLabel})`);
      alert("Weekly team brief saved to Notes.");
    } catch (e: any) {
      console.error("Save weekly team brief failed", e);
      alert(`Failed to save weekly brief: ${e.message || JSON.stringify(e)}`);
    }
  };
  ...
  const savePartnerUpdate = async () => {
    ...
    const payload = {
      tag: "Operations",
      title: `Team Update - ${new Date().toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}`,
      body: `${actor}: ${message}`,
      date: dateLabel,
      files: [],
    };
    ...
    try {
      ...
      logActivity("Posted team update note");
    } catch (e: any) {
      console.error("Save team update failed", e);
      alert(`Couldn't save team update: ${e?.message || "Unknown error"}`);
    }
  };
  ...
  const TABS = [
    ...
    { id: "inspiration", label: "Inspiration Library", icon: FileText, group: "PLANNING" },
    { id: "marketing", label: "Marketing", icon: Megaphone, group: "PLANNING" },
    { id: "decisions", label: "Decision Log", icon: FileText, group: "PLANNING" },
    ...
    { id: "vendors", label: "Vendors & Contacts", icon: Users, group: "OPERATIONS" },
  ...
  return (
    <div style={{ minHeight:"100dvh", background:"transparent", color:T.text, fontFamily:"var(--font-sans)", paddingBottom: isMobile ? "calc(64px + env(safe-area-inset-bottom, 0px))" : "env(safe-area-inset-bottom, 0px)" }}>
      {/* ���� LOGIN SCREEN ���� */}
      {isAuthLoading ? (
        ...
        <>
          {/* ���� HEADER ���� */}
          <div className="glass" style={{ borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, zIndex: 100, paddingTop: "calc(env(safe-area-inset-top, 0px) + 16px)", boxShadow: "0 8px 20px rgba(15, 28, 42, 0.06)" }}>
            {/* Critical Banner */}
            {criticalOverdue > 0 && (
              <div style={{ 
          ...
          <div style={{ display: "flex", gap: isMobile ? 10 : 8, alignItems: "center", justifyContent: isMobile ? "space-between" : "flex-end", width: isMobile ? "100%" : "auto" }}>
            {isUnlocked && (
              <button 
                ...
                style={{ background: "#F4F4F5", border: `1px solid ${T.borderStrong}`, borderRadius: 10, padding: isMobile ? "6px 10px" : "6px 12px", cursor: "pointer", color: T.text, display: "flex", alignItems: "center", gap: 6 }}
              >
                <ShieldCheck size={14} />
                <span style={{ fontSize: 12, fontWeight: 700, fontFamily: "var(--font-sans)" }}>LOCK</span>
              </button>
            )}
            <button 
              onClick={() => setIsUnlocked(false)}
              style={{ background: "#FFFFFF", border: `1px solid ${T.borderStrong}`, borderRadius: 10, padding: isMobile ? "8px 12px" : "8px 16px", fontSize: 13, fontWeight: 700, color: T.text, cursor: "pointer", whiteSpace: "nowrap", boxShadow: "0 1px 4px rgba(15, 28, 42, 0.08)" }}
            >
              Lock Sensitive Views
            </button>
            {!isMobile && <span style={{ fontFamily: "var(--font-sans)", fontSize: 13, color: T.subtle, fontWeight: 700 }}>{new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "America/Phoenix" })}</span>}
          </div>

          {/* AI Search Bar */}
          <div style={{ display: "flex", alignItems: "center", background: "#FFFFFF", borderRadius: 14, padding: isMobile ? "10px 14px" : "10px 14px", border: `1px solid ${T.borderStrong}`, width: isMobile ? "100%" : 360, height: isMobile ? 50 : 48, marginLeft: isMobile ? 0 : 24, boxShadow: "0 4px 12px rgba(15, 28, 42, 0.08)", transition: "all .2s" }}>
            <Sparkles size={15} style={{ marginRight: 10, opacity: 0.85, flexShrink: 0 }} />
            <input 
              placeholder="Ask assistant..." 
              style={{ background: "none", border: "none", outline: "none", fontSize: isMobile ? 15 : 15, width: "100%", color: T.text, fontFamily: "var(--font-sans)", fontWeight: 600 }}
              onKeyDown={(e) => {
        ...
        {!isMobile && (
          <div style={{ display: "flex", padding: "0 24px", gap: 24, height: 46, alignItems: "center" }}>
            {GROUPS.map(g => (
              <button 
                key={g} 
                onClick={() => {
                ...
                style={{ 
                  background: "none", border: "none", borderBottom: `2px solid ${activeGroup === g ? T.text : "transparent"}`,
                  padding: "16px 0", cursor: "pointer", fontSize: 15, fontFamily: "var(--font-sans)",
                  color: activeGroup === g ? T.text : T.muted, fontWeight: activeGroup === g ? 800 : 600,
                  letterSpacing: 0.5, transition: "all .2s",
                  opacity: activeGroup === g ? 1 : 0.7
                }}
              >
                {g.toUpperCase()}
              </button>
      ...
      {!isMobile && (
        <div style={{ display: "flex", background: "#FAFAFA", padding: "0 24px", gap: 8, borderBottom: `1px solid ${T.border}` }}>
            ...
            return (
              <button 
                ...
                style={{ 
                  ...
                  display: "flex", alignItems: "center", gap: 10, color: active ? T.text : T.muted,
                  fontSize: 13, fontWeight: active ? 700 : 600, transition: "all .2s",
                  borderBottom: `3px solid ${active ? T.text : "transparent"}`,
                  marginBottom: -1,
        ...
        {/* Mobile Menu Overlay (full tab list via "More") */}
        {isMobile && isMenuOpen && (
          <div style={{ position:"fixed", inset:0, top:"calc(80px + env(safe-area-inset-top, 0px))", background:"#FFF", zIndex:150, padding:"16px 16px calc(80px + env(safe-area-inset-bottom, 0px))", display:"flex", flexDirection:"column", gap:4, overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
            <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:T.subtle, letterSpacing:1.2, fontWeight:600, padding:"0 4px", marginBottom:8 }}>ALL SECTIONS</div>
            {GROUPS.map(group => (
              <div key={group} style={{ marginBottom:12 }}>
                <div style={{ fontSize:10, fontFamily:"'IBM Plex Mono',monospace", color:T.subtle, letterSpacing:1, padding:"0 8px", marginBottom:6 }}>{group}</div>
                {TABS.filter(t => t.group === group).map(t => (
                  <button
                    key={t.id}
                    onClick={() => { setTab(t.id); setIsMenuOpen(false); }}
                    style={{ textAlign:"left", background:tab===t.id ? T.champagne : "none", border:"none", padding:"13px 12px", borderRadius:10, color:T.text, fontWeight:tab===t.id ? 700 : 500, fontSize:15, lineHeight:1.35, width:"100%", display:"flex", alignItems:"center", gap:12, transition:"background 0.1s" }}
                  >
                    <t.icon size={18} color={tab===t.id ? T.text : T.subtle} />
                    {t.label}
                  </button>
                ))}
              </div>
        ...
        {isMobile && (
          <nav className="bottom-nav no-print">
            ...
            >
              ...
              <span>{isMenuOpen ? "Close" : "More"}</span>
            </button>
          </nav>
        )}
      ...
      {todoModal && (
        <Modal title={todoModal === "new" ? "Add Todo Item" : "Edit Todo Item"} onClose={() => setTodoModal(null)} width={460}>
          ...
          <Field label="ASSIGNED TO"><input value={todoDraft.assignedTo} onChange={e => setTodoDraft(d => ({ ...d, assignedTo: e.target.value }))} style={inpStyle} placeholder="Owner, Manager, Team" /></Field>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20, flexWrap: "wrap" }}>
            {todoModal !== "new" && (
              <div style={{ marginRight: "auto" }}>
                <Btn onClick={() => deleteTaskTodo(todoModal)} variant="outline" style={{ color: T.red, borderColor: T.redBorder }} title="Delete todo"><Trash2 size={13} /></Btn>
              </div>
            )}
      ...
      {noteDetail && <NoteDetailModal note={noteDetail} tasks={tasks} onClose={() => setNoteDetail(null)} onSave={saveNote} onDelete={(id) => {
      ...
      {decisionModal && (
        <Modal title={decisionModal === "new" ? "New Decision" : "Edit Decision"} onClose={() => setDecisionModal(null)} width={560}>
          <Field label="TITLE"><input value={decisionDraft.title} onChange={e => setDecisionDraft((f: any) => ({ ...f, title: e.target.value }))} style={inpStyle} placeholder="e.g. Launch weekend menu scope" /></Field>
          <Field label="DECISION"><textarea value={decisionDraft.decision} onChange={e => setDecisionDraft((f: any) => ({ ...f, decision: e.target.value }))} rows={3} style={{ ...inpStyle, resize: "vertical", lineHeight: 1.5 }} placeholder="What was decided and why?" /></Field>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="OWNER"><input value={decisionDraft.owner} onChange={e => setDecisionDraft((f: any) => ({ ...f, owner: e.target.value }))} style={inpStyle} placeholder="Owner / Team / Manager" /></Field>
            <Field label="DATE"><input type="date" value={decisionDraft.date} onChange={e => setDecisionDraft((f: any) => ({ ...f, date: e.target.value }))} style={inpStyle} /></Field>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label="IMPACT">
              <select value={decisionDraft.impact} onChange={e => setDecisionDraft((f: any) => ({ ...f, impact: e.target.value }))} style={{ ...inpStyle, height: 42 }}>
                {["Low", "Medium", "High"].map(v => <option key={v} value={v}>{v}</option>)}
              </select>
            </Field>
            <Field label="STATUS">
              <select value={decisionDraft.status} onChange={e => setDecisionDraft((f: any) => ({ ...f, status: e.target.value }))} style={{ ...inpStyle, height: 42 }}>
                {["Proposed", "Decided", "Revisit"].map(v => <option key={v} value={v}>{v}</option>)}
              </select>
            </Field>
          </div>
          <Field label="NEXT STEP (OPTIONAL)"><input value={decisionDraft.nextStep || ""} onChange={e => setDecisionDraft((f: any) => ({ ...f, nextStep: e.target.value }))} style={inpStyle} placeholder="Who does what next and by when?" /></Field>
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
            <Btn onClick={() => setDecisionModal(null)} variant="ghost">Cancel</Btn>
            <Btn onClick={() => { if (decisionDraft.title.trim() && decisionDraft.decision.trim()) saveDecision(decisionDraft); }} variant="primary">{decisionModal === "new" ? "Add Decision" : "Save Decision"}</Btn>
          </div>
        </Modal>
      )}
      {vendorModal && <VendorModal vendor={vendorModal === "new" ? null : vendorModal} onSave={saveVendor} onClose={() => setVendorModal(null)} />}
      {contactModal && <ContactModal contact={contactModal === "new" ? null : contactModal} onSave={saveContact} onClose={() => setContactModal(null)} />}
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,.3)",zIndex:998,display:"flex",alignItems:"center",justifyContent:"center" }} onClick={()=>setDelConfirm(null)}>
          <div style={{ background:"#FFF",borderRadius:14,padding:26,width:340,boxShadow:"0 16px 40px rgba(0,0,0,.12)" }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ display:"flex",gap:10,justifyContent:"flex-end" }}>
              ...
              <Btn onClick={()=>{delConfirm.onConfirm();setDelConfirm(null);}} variant="danger" title="Delete"><Trash2 size={13} /></Btn>
            </div>
          </div>
        </div>
      )}

      <div style={{ padding: isMobile ? "16px" : "22px 24px", maxWidth: 1360, margin: "0 auto", display: "flex", gap: 24, paddingBottom: isMobile ? "96px" : undefined }}>
          ...
          <div className="fu" style={isMobile ? { display: "flex", flexDirection: "column" } : undefined}>
            ...
            <div style={{ order: isMobile ? 5 : 2, background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 16, padding: isMobile ? 12 : 18, marginBottom: isMobile ? 14 : 18 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(4,1fr)" : "repeat(6,1fr)", gap: isMobile ? 8 : 10 }}>
                {[
                  ...
                  { label: "Menu",       icon: Utensils,     tab: "menu" },
                  { label: "Timeline",   icon: MapPin,       tab: "timeline" },
                  { label: "Decisions",  icon: MessageSquare,tab: "decisions" },
            ...
            <div style={{ order: isMobile ? 2 : 3, background: PASTEL.sky.bg, border: `1px solid ${PASTEL.sky.border}`, borderRadius: 14, padding: isMobile ? "12px 14px" : "14px 18px", marginBottom: isMobile ? 14 : 18, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Photo Album</div>
                  <div style={{ fontSize: 12, color: T.subtle }}>All your restaurant photos organized in Dropbox</div>
                </div>
              </div>
              <a
                href="https://www.dropbox.com/scl/fo/2bqe785ejhqtovanktrb7/AMn-iAg9rAWSmq7yXP7uQyE?rlkey=fm5kpcuktza3p1y2l35kz7huq&st=gjgw1few&dl=0"
                target="_blank"
                rel="noreferrer"
                style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "#111", color: "#FFF", border: "none", borderRadius: 10, padding: "10px 16px", fontWeight: 700, fontSize: 13, textDecoration: "none" }}
              >
            ...
            <div style={{ order: isMobile ? 1 : 6, display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 14 : 18, marginBottom: isMobile ? 14 : 18 }}>
              ...
              <div style={{ flex: 1, background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 16, padding: isMobile ? "14px" : 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 10, flexWrap: "wrap" }}>
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: T.muted, letterSpacing: 1.2, fontWeight: 600 }}>QUICK UPDATE</div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {partnerUpdates.length === 0 ? (
                    <div style={{ fontSize: 12, color: T.muted, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 10, padding: "10px 12px" }}>
                      No updates yet — post your first one above.
                    </div>
                  ) : partnerUpdates.slice(0, 4).map((update, idx) => (
                    <div key={`partner-update-${update.id}`} style={{ border: `1px solid ${T.border}`, borderRadius: 10, padding: "9px 12px", background: "#FFF", display: "grid", gridTemplateColumns: "22px 1fr", gap: 8 }}>
                      <div style={{ width: 22, height: 22, borderRadius: 99, border: `1px solid ${T.border}`, background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: T.subtle, fontWeight: 700 }}>{idx + 1}</div>
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 700, color: T.text, overflowWrap: "anywhere" }}>{update.title}</div>
                        <div style={{ fontSize: 12, color: T.subtle, marginTop: 3, overflowWrap: "anywhere" }}>{update.body}</div>
                        <div style={{ fontSize: 10, color: T.muted, marginTop: 4 }}>{update.date || ""}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ order: isMobile ? 6 : 7, background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 16, padding: isMobile ? "12px" : 18, marginBottom: isMobile ? 14 : 18 }}>
              ...
              <LaunchWindow
                ...
                onOpenDay={(dateStr) => { setCalendarFocusDate(dateStr); setTab("calendar"); }}
              />
            </div>
            ...
            <div style={{ order: isMobile ? 8 : 9, background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 16, padding: isMobile ? 12 : 18, marginBottom: isMobile ? 14 : 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 10, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <Btn onClick={copyWeeklyPartnerBrief} variant="outline" small>Copy Brief</Btn>
        ...
        {tab==="tasks" && (
          <div className="fu">
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16, alignItems: "start" }}>
              <div style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 16, boxShadow: "0 1px 2px rgba(17, 17, 17, 0.05)", overflow: "hidden", minWidth: 0 }}>
                    ...
                    actionListTodos.map(todo => {
                      ...
                      return (
                        <div key={todo.id} style={{ borderBottom: `1px solid ${T.border}`, padding: "10px 12px", background: isDone ? T.bg : "#FFF", display: "grid", gridTemplateColumns: "3px 1fr", gap: 10 }}>
                          <div style={{ background: isDone ? T.subtle : T.border, borderRadius: 99 }} />
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                            <button
                              onClick={() => toggleTodoDone(todo)}
                              style={{ width: isMobile ? 22 : 18, height: isMobile ? 22 : 18, minWidth: isMobile ? 22 : 18, border: `2px solid ${isDone ? T.text : T.border}`, borderRadius: 5, background: isDone ? T.bg : "#FFF", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: "pointer", padding: 0, marginTop: 1, transition: "all .15s" }}
                            >
                              {isDone && <span style={{ color: T.text, fontSize: 11, fontWeight: 700, lineHeight: 1 }}>✓</span>}
                            </button>
                            <div style={{ minWidth: 0, flex: 1 }}>
                              <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start" }}>
                                ...
                                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                                  ...
                                  <button onClick={() => deleteTaskTodo(todo)} title="Delete" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 8, padding: isMobile ? "8px 10px" : "5px 7px", color: T.muted, cursor: "pointer", display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                                </div>
                              </div>
                              {todo.category && <div style={{ marginTop: 3, fontSize: 10, color: T.muted, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 99, padding: "1px 7px", display: "inline-block", fontWeight: 600 }}>{todo.category}</div>}
                                    ...
                                    return (
                                      <div key={sub.id} style={{ display: "flex", alignItems: "center", gap: 6, border: `1px solid ${T.border}`, borderRadius: 8, background: "#FFF", padding: isMobile ? "8px 10px" : "6px 8px" }}>
                                        <input
                                          ...
                                          style={{ width: isMobile ? 16 : 13, height: isMobile ? 16 : 13, accentColor: T.text, flexShrink: 0 }}
                                        />
                                        {isEditing ? (
                                          <input
                                            value={draftValue}
                                        ...
                                        <button onClick={() => deleteTodoSubtask(todo, sub.id)} title="Delete subtask" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 7, padding: isMobile ? "6px 8px" : "3px 5px", color: T.muted, cursor: "pointer", display: "flex", alignItems: "center", flexShrink: 0 }}><Trash2 size={11} /></button>
                                      </div>
                                    );
                                  })}
              ...
              <div style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 16, padding: isMobile ? 12 : 16, minWidth: 0 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <button
                    ...
                    title="Toggle dependency lock for master task sequencing"
                  >
                    Dependency Lock: {dependencyLockEnabled ? "ON" : "OFF"}
                  </button>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 12 }}>
                {categoryTimelineCards.map(card => (
                  <button
                    key={`timeline-card-${card.category}`}
                    onClick={() => {
                      setCatF(card.category);
                      setStatF("All");
                    }}
                    style={{
                      textAlign: "left",
                      cursor: "pointer",
                      borderRadius: 12,
                      border: `1px solid ${catFilter === card.category ? T.text : T.border}`,
                      background: catFilter === card.category ? T.bg : "#FFF",
                      padding: "10px 12px",
                    }}
                  >
                    <div style={{ fontSize: 12, color: T.text, fontWeight: 700, marginBottom: 5 }}>{card.category}</div>
                    <div style={{ fontSize: 11, color: T.muted, marginBottom: 6 }}>{card.open} open / {card.total} total</div>
                    <div style={{ height: 5, borderRadius: 999, background: T.border, overflow: "hidden", marginBottom: 6 }}>
                      <div style={{ width: `${card.progress}%`, height: "100%", background: T.text, borderRadius: 999 }} />
                    </div>
                    <div style={{ fontSize: 10, color: T.subtle }}>
                      {card.nextDue ? `Next due ${new Date(`${card.nextDue}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric" })}` : "No due date set"}
                    </div>
                  </button>
                ))}
              </div>
              ...
              {catFilter !== "All" && (
                <div style={{ border: `1px solid ${T.border}`, borderRadius: 12, padding: "12px", background: T.bg, marginBottom: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                    <div style={{ fontSize: 12, color: T.text, fontWeight: 700 }}>{catFilter} Task Sequence</div>
                    <Btn onClick={() => setCatF("All")} variant="ghost" small>Show All Categories</Btn>
                  </div>
                  {categorySequenceTasks.length === 0 ? (
                    <div style={{ fontSize: 12, color: T.muted }}>No tasks in this category yet.</div>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {categorySequenceTasks.map((task, index) => (
                        (() => {
                          const blockedReason = blockedTaskReasonById[task.id];
                          ...
                          const isLast = index === categorySequenceTasks.length - 1;
                          return (
                        <div key={`sequence-${task.id}`} style={{ display: "grid", gridTemplateColumns: "30px 1fr", gap: 10, alignItems: "stretch" }}>
                          <div style={{ position: "relative" }}>
                            <div style={{ width: 22, height: 22, borderRadius: 99, border: `1px solid ${T.border}`, background: "#FFF", color: T.text, fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", margin: "1px auto 0" }}>
                              {index + 1}
                            </div>
                            {!isLast && <div style={{ position: "absolute", left: "50%", top: 25, bottom: -11, width: 1, background: T.border, transform: "translateX(-50%)" }} />}
                          </div>
                          ...
                          <div style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: "10px 11px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, flexWrap: "wrap" }}>
                              ...
                              <div style={{ display: "flex", gap: 6 }}>
                                ...
                                <button
                                  onClick={() => promptDelayFromTask(task)}
                                  style={{ cursor: "pointer", borderRadius: 8, border: `1px solid ${T.border}`, background: "#FFF", color: T.muted, fontSize: 10, padding: "4px 8px" }}
                                >
                                  Delay From Here
                                </button>
                              </div>
                            ...
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 7 }}>
                              <span style={{ fontSize: 10, borderRadius: 99, border: `1px solid ${T.border}`, background: T.bg, color: T.muted, padding: "2px 8px" }}>{task.status}</span>
                              <span style={{ fontSize: 10, borderRadius: 99, border: `1px solid ${T.border}`, background: T.bg, color: T.muted, padding: "2px 8px" }}>{task.priority}</span>
                              <span style={{ fontSize: 10, borderRadius: 99, border: `1px solid ${T.border}`, background: T.bg, color: T.muted, padding: "2px 8px" }}>
                                {task.due ? `Due ${new Date(`${task.due}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric" })}` : "No due date"}
                              </span>
                              {blockedReason && <span style={{ fontSize: 10, borderRadius: 99, border: `1px solid ${T.border}`, background: "#FFF", color: T.subtle, padding: "2px 8px" }}>Locked</span>}
                            </div>

                            {totalSubtasks > 0 && (
                              <div style={{ marginTop: 8 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                                  <div style={{ fontSize: 10, color: T.subtle }}>Checklist Progress</div>
                                  <div style={{ fontSize: 10, color: T.muted }}>{doneSubtasks}/{totalSubtasks}</div>
                                </div>
                                <div style={{ height: 4, borderRadius: 999, background: T.border, overflow: "hidden" }}>
                                  <div style={{ height: "100%", width: `${subtaskPct}%`, background: T.text, borderRadius: 999 }} />
                                </div>
                              </div>
                            )}

                            {blockedReason && <div style={{ fontSize: 10, color: T.subtle, marginTop: 6 }}>{blockedReason}</div>}

                            {task.checklist.length > 0 && (
                              <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
                                {task.checklist.slice(0, 3).map(item => (
                                  <div key={`sequence-sub-${task.id}-${item.id}`} style={{ fontSize: 10, color: item.done ? T.subtle : T.muted, textDecoration: item.done ? "line-through" : "none", overflowWrap: "anywhere" }}>
                                    {item.done ? "[x]" : "[ ]"} {item.text}
                                  </div>
                                ))}
                                {task.checklist.length > 3 && <div style={{ fontSize: 10, color: T.subtle }}>+{task.checklist.length - 3} more subtasks</div>}
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {["All", ...CATEGORIES].map(category => (
                      <button
                        ...
                        style={{ cursor: "pointer", borderRadius: 999, padding: "4px 10px", fontSize: 10, border: `1px solid ${catFilter === category ? T.text : T.border}`, background: catFilter === category ? T.bg : "#FFF", color: catFilter === category ? T.text : T.muted, fontWeight: catFilter === category ? 700 : 500 }}
                      >
                        {category}
                      </button>
                    ))}
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {["All", "Not Started", "In Progress", "Complete", "Overdue"].map(status => (
                      <button
                        ...
                        style={{ cursor: "pointer", borderRadius: 999, padding: "4px 10px", fontSize: 10, border: `1px solid ${statFilter === status ? T.text : T.border}`, background: statFilter === status ? T.bg : "#FFF", color: statFilter === status ? T.text : T.muted, fontWeight: statFilter === status ? 700 : 500 }}
                      >
                        {status}
                      </button>
                    ))}
              ...
              {catFilter !== "All" && (
                <div style={{ fontSize: 11, color: T.muted, marginBottom: 10 }}>
                  Showing ordered tasks and subtasks for <strong style={{ color: T.text }}>{catFilter}</strong>. Use "Delay From Here" to push later tasks back together.
                </div>
              )}
              ...
              {isMobile ? (
                          ...
                          <div style={{ padding: "14px 16px 0" }}>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start", marginBottom: 10 }}>
                              ...
                              <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                                ...
                                <button onClick={() => setDelConfirm({ label: task.task, onConfirm: () => deleteRecord('tasks', task.id, task.task, setTasks) })} title="Delete" style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 10, padding: "8px 10px", color: T.muted, cursor: "pointer", display: "flex", alignItems: "center" }}><Trash2 size={15} /></button>
                              </div>
                            </div>

                            {/* Meta badges */}
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                              <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 99, background: T.bg, color: T.muted, border: `1px solid ${T.border}`, fontWeight: 600 }}>{task.category}</span>
                              <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 99, background: T.bg, color: T.muted, border: `1px solid ${T.border}`, fontWeight: 600 }}>{task.status}</span>
                              <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 99, background: T.bg, color: T.muted, border: `1px solid ${T.border}`, fontWeight: 700 }}>{task.priority}</span>
                              ...
                              {isBlocked && <span style={{ fontSize: 11, padding: "4px 10px", borderRadius: 99, background: "#FFF", color: T.subtle, border: `1px solid ${T.border}` }}>Locked</span>}
                            </div>
                            {blockedReason && <div style={{ fontSize: 11, color: T.subtle, marginBottom: 10 }}>{blockedReason}</div>}
                            ...
                            {total > 0 && (
                              <div style={{ marginBottom: 12 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                                  ...
                                  <span style={{ fontSize: 11, color: T.muted, fontWeight: 600 }}>{done}/{total}</span>
                                </div>
                                <div style={{ height: 6, borderRadius: 999, background: T.bg, overflow: "hidden" }}>
                                  <div style={{ height: "100%", width: `${pct}%`, background: T.text, transition: "width .3s ease", borderRadius: 999 }} />
                                </div>
                              </div>
                            )}
                          </div>
                            ...
                            <div style={{ overflow: "hidden" }}>
                                ...
                                {task.checklist.map(item => {
                                  ...
                                  return (
                                    <div key={`task-mobile-check-${task.id}-${item.id}`} style={{ display: "flex", alignItems: "center", gap: 10, background: "#FFF", borderRadius: 12, padding: "10px 12px", border: `1px solid ${T.border}` }}>
                                      <button onClick={() => { if (isBlocked) return; toggleTaskCheck(task.id, item.id); }} style={{ width: 22, height: 22, minWidth: 22, border: `2px solid ${item.done ? T.text : T.border}`, borderRadius: 6, background: item.done ? T.bg : "#FFF", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: isBlocked ? "not-allowed" : "pointer", padding: 0, opacity: isBlocked ? 0.5 : 1 }}>
                                        {item.done && <span style={{ color: T.text, fontSize: 12, fontWeight: 700 }}>✓</span>}
                                      </button>
                                      {isEditing ? (
                                        <input
                                          value={draftValue}
                                          onChange={e => { if (isBlocked) return; setEditingTaskSubtaskDrafts(prev => ({ ...prev, [editKey]: e.target.value })); }}
                                          onKeyDown={e => {
                                            if (isBlocked) return;
                                            if (e.key === "Enter") {
                                      ...
                                      <button onClick={() => { if (isBlocked) return; setEditingTaskSubtaskDrafts(prev => ({ ...prev, [editKey]: item.text })); }} style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 8, padding: 7, color: T.muted, cursor: isBlocked ? "not-allowed" : "pointer", display: "flex", alignItems: "center", opacity: isBlocked ? 0.5 : 1 }}><PenLine size={13} /></button>
                                      <button onClick={() => { if (isBlocked) return; deleteTaskSubtask(task.id, item.id); }} style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 8, padding: 7, color: T.muted, cursor: isBlocked ? "not-allowed" : "pointer", display: "flex", alignItems: "center", opacity: isBlocked ? 0.5 : 1 }}><Trash2 size={13} /></button>
                                    </div>
                                  );
                                })}
                              </div>
                              <div style={{ display: "flex", gap: 8 }}>
                                <input
                                  ...
                                  onChange={e => { if (isBlocked) return; setSubtaskDrafts(prev => ({ ...prev, [task.id]: e.target.value })); }}
                                  onKeyDown={e => {
                                    if (isBlocked) return;
                                    if (e.key === "Enter") {
                                  ...
                                  placeholder="Add subtask..."
                                  style={{ ...inpStyle, fontSize: 13, padding: "10px 14px", flex: 1, borderRadius: 12, opacity: isBlocked ? 0.6 : 1 }}
                                  disabled={isBlocked}
                                />
                                <Btn onClick={() => {
                                  if (isBlocked) return;
                ...
                <div style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden", minWidth: 0 }}>
                  {orderedFilteredTasks.length === 0 ? (
                    ...
                    orderedFilteredTasks.map(task => (
                      <TaskRow
                        ...
                        onDelayFromHere={promptDelayFromTask}
                        blockedReason={blockedTaskReasonById[task.id]}
                      />
                    ))
                  )}
                </div>
                ...
                {menuItems.filter(m => menuSecF === "All" || m.section === menuSecF).map(item => (
                  <div key={item.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 10, justifyContent: "flex-end" }}>
                      <button onClick={() => setMenuModal(item)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center" }}><PenLine size={13} /></button>
                      <button onClick={() => setDelConfirm({ label: item.name, onConfirm: () => deleteRecord('menu_items', item.id, item.name, setMenuItems) })} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "4px 8px", color: T.red, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center" }}><Trash2 size={13} /></button>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {menuItems.filter(m=>menuSecF==="All"||m.section===menuSecF).map((item,i)=>(
                  <div key={item.id} style={{ display:"grid", gridTemplateColumns:"110px 44px 1fr 1.8fr 70px 80px 52px 1fr 70px", padding:"13px 18px", borderBottom:`1px solid ${T.border}`, background:i%2===0?"#FFF":T.bg, alignItems:"center" }}>
                    ...
                    <div style={{ display:"flex", gap:5 }}>
                      <button onClick={()=>setMenuModal(item)} title="Edit" style={{ background:"none",border:`1px solid ${T.border}`,borderRadius:6,padding:"3px 7px",color:T.muted,cursor:"pointer",fontSize:11,display:"flex",alignItems:"center" }}><PenLine size={12} /></button>
                      <button onClick={()=>setDelConfirm({label:item.name,onConfirm:()=>deleteRecord('menu_items', item.id, item.name, setMenuItems)})} title="Delete" style={{ background:"none",border:`1px solid ${T.redBorder}`,borderRadius:6,padding:"3px 7px",color:T.red,cursor:"pointer",fontSize:11,display:"flex",alignItems:"center" }}><Trash2 size={12} /></button>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  consolidatedShoppingRows.map((ing, i) => (
                    <div key={ing.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: 12 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                        <button onClick={() => openEditShopItem(ing)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><PenLine size={12} /></button>
                        <button onClick={() => removeShopItem(ing)} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "5px 8px", color: T.red, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                      </div>
                    </div>
                  ))
                )}
                ...
                {consolidatedShoppingRows.length === 0 ? (
                  ...
                  consolidatedShoppingRows.map((ing) => (
                    <div key={ing.id} style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr 1.2fr 120px 120px 1.8fr 130px", padding: "14px 18px", borderBottom: `1px solid ${T.border}`, alignItems: "center" }}>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => openEditShopItem(ing)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><PenLine size={12} /></button>
                        <button onClick={() => removeShopItem(ing)} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "4px 8px", color: T.red, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                      </div>
                    </div>
                  ))
                )}
                    ...
                    <div key={c.id} style={{ marginBottom:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5, alignItems:"center" }}>
                        ...
                        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                          ...
                          <button onClick={()=>setFinModal({type:"startup",item:c})} title="Edit" style={{ background:"none",border:"none",color:T.muted,cursor:"pointer",fontSize:14,padding:"2px 4px",lineHeight:1,display:"flex",alignItems:"center" }}><PenLine size={13} /></button>
                          <button onClick={()=>setDelConfirm({label:c.category,onConfirm:()=>deleteRecord('startup_costs', c.id, c.category, setStartup)})} title="Delete" style={{ background:"none",border:"none",color:T.red,cursor:"pointer",fontSize:14,padding:"2px 4px",lineHeight:1,display:"flex",alignItems:"center" }}><Trash2 size={13} /></button>
                        </div>
                      </div>
                ...
                {operatingCostsWithUtilities.map(c=>(
                  <div key={c.id} style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:`1px solid ${T.border}`, alignItems:"center" }}>
                    ...
                    <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                      ...
                      <button onClick={()=>setFinModal({type:"operating",item:c})} title="Edit" style={{ background:"none",border:"none",color:T.muted,cursor:"pointer",fontSize:14,padding:"2px 4px",display:"flex",alignItems:"center" }}><PenLine size={13} /></button>
                      <button onClick={()=>setDelConfirm({label:c.category,onConfirm:()=>deleteRecord('operating_costs', c.id, c.category, setOp)})} title="Delete" style={{ background:"none",border:"none",color:T.red,cursor:"pointer",fontSize:14,padding:"2px 4px",display:"flex",alignItems:"center" }}><Trash2 size={13} /></button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill,minmax(320px,1fr))", gap: 14 }}>
              {decisionLogs.length === 0 ? (
                <div style={{ gridColumn: "1/-1", padding: 40, background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 14, color: T.muted, textAlign: "center" }}>
                  No decisions logged yet. Add one to document team alignment.
                </div>
              ) : decisionLogs
                .slice()
                .sort((a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime())
                .map(item => {
                  const impactColor = item.impact === "High" ? T.red : item.impact === "Medium" ? T.gold : T.blue;
                  const statusBg = item.status === "Decided" ? T.greenLight : item.status === "Revisit" ? T.goldLight : T.bg;
                  const statusColor = item.status === "Decided" ? T.green : item.status === "Revisit" ? T.gold : T.muted;
                  return (
                    <div key={item.id} style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 14, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                        <div style={{ fontSize: 16, fontWeight: 700, color: T.text, lineHeight: 1.3 }}>{item.title}</div>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button onClick={() => openEditDecision(item)} title="Edit" style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "4px 8px", color: T.muted, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><PenLine size={12} /></button>
                          <button onClick={() => setDelConfirm({ label: item.title, onConfirm: () => deleteRecord("decision_logs", item.id, item.title, setDecisionLogs as any) })} title="Delete" style={{ background: "none", border: `1px solid ${T.redBorder}`, borderRadius: 6, padding: "4px 8px", color: T.red, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center" }}><Trash2 size={12} /></button>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <span style={{ fontSize: 10, borderRadius: 99, border: `1px solid ${T.border}`, background: statusBg, color: statusColor, padding: "3px 8px", fontWeight: 700 }}>{item.status}</span>
                        <span style={{ fontSize: 10, borderRadius: 99, border: `1px solid ${impactColor}55`, background: `${impactColor}22`, color: impactColor, padding: "3px 8px", fontWeight: 700 }}>{item.impact} impact</span>
                      </div>
                      <div style={{ fontSize: 13, color: T.text, lineHeight: 1.6 }}>{item.decision}</div>
                      <div style={{ fontSize: 11, color: T.muted }}>Owner: {item.owner || "Unassigned"} · Date: {item.date || "TBD"}</div>
                      {item.nextStep && (
                        <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 10, padding: "8px 10px", fontSize: 12, color: T.muted }}>
                          Next: {item.nextStep}
                        </div>
                      )}
                    </div>
        ...
        {tab === "vendors" && (
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 20, alignItems: "start" }}>
            <VendorManager vendors={vendors} onAdd={() => setVendorModal("new")} onEdit={setVendorModal} onDelete={id => {
              const v = vendors.find(x => x.id === id);
              const label = v?.name || "Vendor";
              setDelConfirm({ label, onConfirm: () => deleteRecord('vendors', id, label, setVendors) });
            }} />
            <ContactManager contacts={contacts} onAdd={() => setContactModal("new")} onEdit={setContactModal} onDelete={id => {
              const c = contacts.find(x => x.id === id);
              const label = c?.name || "Contact";
              setDelConfirm({ label, onConfirm: () => deleteRecord('contacts', id, label, setContacts) });
            }} />
          </div>
        )}
          ...
          !isUnlocked ? <PinGate onUnlock={() => setIsUnlocked(true)} correctPin={securityPin} /> : !canManageAccess ? (
            <div className="fu">
              ...
              <div style={{ background: "#FFF", border: `1px solid ${T.border}`, borderRadius: 12, padding: 20, textAlign: "center" }}>
                <p style={{ color: T.muted, fontSize: 13 }}>Only owner and team accounts can access settings.</p>
              </div>
            </div>
          ) : (
            <div className="fu">
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {accessUsers.map((u) => (
                      <div key={u.id} style={{ border: `1px solid ${T.border}`, borderRadius: 10, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                        <div>
                          <div style={{ fontSize: 13, color: T.text, fontWeight: 600 }}>{String(u.email || "").toLowerCase() === "yanajaib@gmail.com" ? "Team" : (u.name || "User")}</div>
